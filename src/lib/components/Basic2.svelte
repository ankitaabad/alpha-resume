...
<div>
  <div
      class="aspect-square w-24 h-24 bg-transparent flex items-center justify-center mx-auto mb-3"
  ...
  <div class="flex gap-4 items-center justify-center mb-3 flex-wrap max-w-screen-sm mx-auto">
      <p class="" class:hidden={basic_data?.Email == ''}>
      ...
      <p class="" class:hidden={$preview_data?.Basic?.data[0].Phone == ''}>
          <a
              href="tel:{$preview_data?.Basic?.data[0].Phone}"
              class="text-gray-500 flex items-center gap-1"
              ><svelte:component this={get_social_icon('phone')} class="inline" />{$preview_data?.Basic
                  ?.data[0].Phone}</a
          >
      </p>
      <p class=" flex items-center gap-1" class:hidden={basic_data?.Location == ''}>
      ...
      {#each $preview_data?.['Social Profile']?.data as socialProfile}
          <p class=" flex items-center gap-1" class:hidden={socialProfile.Username == ''}>
              <svelte:component
